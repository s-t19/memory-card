...
skip.setStyleSheet("background-color: lightblue;")

rest = QPushButton('Рестарт')
...
layout_line3.addWidget(skip, stretch=2)
layout_line3.addStretch(1)
...
def check_answer():
    ...
    #print('- Всего пропущено:', window.skipol)
...
#s = QLabel("Выберите авриант ответа")

def click_ok():
    if btn_OK.text() == 'Следующий вопрос' or answers[0].isChecked() or answers[1].isChecked() or answers[2].isChecked() or answers[3].isChecked():
        skip.show()
...
skip.clicked.connect(next_question)
rest.clicked.connect(restart)
